// SD COMAYAGUA · Corrección comercial estable V4
(() => {
  'use strict';
  ...
  const state = { qty: 1, stock: 1, price: 0, cardPrice: 0, cardName: '', location: '', delivery: '' };
  ...
  function matches(rule, productName) {
  ...
  function renderSelectors(p) {
    ...
    if (state.location === 'local') {
      ...
      delivery.innerHTML = `<div class="detail-local-delivery-summary"><div><span>Entrega en casco urbano</span><small>Envío: ${fmt(LOCAL)}</small></div><strong>${fmt(p.localTotal)}</strong></div>`;
      return;
    }
    if (hint) hint.textContent = 'Ahora selecciona Envío normal o Pagar al recibir.';
    delivery.innerHTML = `<div class="detail-delivery-copy"><strong>Elige el tipo de envío</strong><small>El total ya incluye producto y envío.</small></div><div class="detail-delivery-options"><button type="button" data-detail-delivery="normal" class="${state.delivery === 'normal' ? 'active' : ''}"><span>ENVÍO NORMAL</span><strong>${fmt(p.normal)}</strong><small>Depósito o transferencia antes de enviar.</small></button><button type="button" data-detail-delivery="receive" class="${state.delivery === 'receive' ? 'active' : ''}"><span>PAGAR AL RECIBIR</span><strong>${fmt(p.receive)}</strong><small>Producto + Lps.110.00, más 10%.</small></button></div>`;
  }
  ...
  function render() {
    ...
    const incomplete = PUBLIC && (!state.location || (state.location === 'national' && !state.delivery));
    const buy = document.getElementById('detailDirectBuyBtn');
    if (buy) {
      buy.disabled = incomplete;
      buy.textContent = !state.location ? 'Selecciona tu ubicación' : state.location === 'national' && !state.delivery ? 'Selecciona el tipo de envío' : 'Comprar por WhatsApp';
    }
  }
  ...
  function send() {
    if (!state.location || (state.location === 'national' && !state.delivery)) return;
    const p = pricing();
    ...
    if (state.location === 'local') lines.push('*Ubicación:* Casco urbano de Comayagua', `*Envío local:* ${fmt(LOCAL)}`, `*Total final:* ${fmt(p.localTotal)}`, 'Aplica únicamente dentro del casco urbano.');
    else if (state.delivery === 'receive') lines.push('*Ubicación:* Fuera de Comayagua', '*Modalidad:* Pagar al recibir', `*Envío:* ${fmt(NATIONAL)}`, `*Total final:* ${fmt(p.receive)}`, 'Producto + envío, más 10%.');
    else lines.push('*Ubicación:* Fuera de Comayagua', '*Modalidad:* Envío normal', `*Envío:* ${fmt(NATIONAL)}`, `*Total final:* ${fmt(p.normal)}`, 'Pago antes de enviar.');
  ...
  function normalizeCard(card) {
    ...
    const offer = tiers.map((item) => ({ min: Number(item.minQty), price: Number(item.price) })).filter((item) => item.price > 0 && item.price < base).sort((a, b) => a.price - b.price)[0] || null;
    const signature = `${base}|${offer ? `${offer.min}:${offer.price}` : 'regular'}`;
    if (card.dataset.commercePriceV4 === signature && card.querySelector('.commerce-card-price')) return;
    ...
    box.innerHTML = offer ? `<div class="commerce-card-price-before"><span>ANTES</span><del>${fmt(base)}</del></div><div class="commerce-card-price-now"><span>AHORA</span><strong>${fmt(offer.price)}</strong><small>desde ${offer.min} ${dedales(productName) ? 'pares' : 'unidades'}</small></div>` : `<div class="commerce-card-price-now"><span>PRECIO</span><strong>${fmt(base)}</strong><small>precio actual</small></div>`;
    card.querySelector('.product-name')?.insertAdjacentElement('afterend', box);
    card.dataset.commercePriceV4 = signature;
  }
  ...
  function click(event) {
    ...
    if (event.target.closest?.('#detailQuantityMinus')) { event.preventDefault(); event.stopImmediatePropagation(); state.qty = Math.max(1, state.qty - 1); render(); return; }
    if (event.target.closest?.('#detailQuantityPlus')) { event.preventDefault(); event.stopImmediatePropagation(); state.qty = Math.min(state.stock, state.qty + 1); render(); return; }
    const locationButton = event.target.closest?.('[data-detail-location]');
    if (locationButton) { event.preventDefault(); event.stopImmediatePropagation(); state.location = locationButton.dataset.detailLocation; state.delivery = state.location === 'local' ? 'local' : ''; render(); return; }
    const deliveryButton = event.target.closest?.('[data-detail-delivery]');
    if (deliveryButton) { event.preventDefault(); event.stopImmediatePropagation(); state.delivery = deliveryButton.dataset.detailDelivery; render(); return; }
    if (event.target.closest?.('#detailDirectBuyBtn')) { event.preventDefault(); event.stopImmediatePropagation(); send(); }
  }
  ...
  function init() {
    ...
    if (dialog) new MutationObserver(() => {
      ...
      [150, 350, 700].forEach((delay) => setTimeout(() => { removeGalleryMarker(); sync(false); }, delay));
