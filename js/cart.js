...
export function renderCartView() {
  ...
  <section class="cart-layout">
    ...
    <aside class="card sdc-cart-summary-card">
      <h2>Resumen</h2>
...
function renderCartLine(item) {
  ...
  const cartImg = fallback
    ? `<img src="${escapeHtml(fallback)}" alt="${escapeHtml(item.nombre)}" loading="lazy">`
    : `<span>${escapeHtml(String(item.nombre || '?').slice(0,1).toUpperCase())}</span>`;
  ...
  return `<div class="cart-line sdc-cart-line-v164">
    <div class="sdc-cart-left">
      <div class="sdc-cart-thumb">${cartImg}</div>
      <div class="cart-line-info"><strong>${escapeHtml(item.nombre)}</strong><br><small>${escapeHtml(item.codigo)}${item.color ? ' · ' + escapeHtml(item.color) : ''}</small></div>
    </div>
    <div class="cart-controls">
      ${colorSelect}
      <div class="cart-qty-stepper" aria-label="Cantidad">
        <button class="mini-btn" type="button" data-cart-qty-minus="${item.lineId}" aria-label="Restar cantidad">−</button>
        <input type="number" min="1" max="${item.availableStock}" value="${item.qty}" data-cart-qty="${item.lineId}" aria-label="Cantidad">
        <button class="mini-btn" type="button" data-cart-qty-plus="${item.lineId}" aria-label="Sumar cantidad">+</button>
      </div>
      <span class="cart-line-total">${formatMoney(pricing.total, state.settings)}${priceNote}</span>
      <button class="mini-btn sdc-remove-line" data-remove-cart="${item.lineId}">Quitar</button>
    </div>
    ${promoHint(pricing)}
  </div>`;
}
...
export function bindCartEvents(root=document) {
  ...
  root.querySelectorAll('[data-cart-qty-plus]').forEach(btn => btn.addEventListener('click', () => {
    const item = state.cart.items.find(i => i.lineId === btn.dataset.cartQtyPlus);
    if (item) updateCartLine(item.lineId, { qty: Number(item.qty || 1) + 1 });
  }));
  root.querySelectorAll('[data-cart-qty-minus]').forEach(btn => btn.addEventListener('click', () => {
    const item = state.cart.items.find(i => i.lineId === btn.dataset.cartQtyMinus);
    if (item) updateCartLine(item.lineId, { qty: Number(item.qty || 1) - 1 });
  }));
