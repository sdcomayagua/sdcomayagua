...
export function renderStatusStrip() {
  ...
  el.innerHTML = `
    <div class="status-pill"><span><span class="dot ${dotClass}"></span> ${state.status.online ? 'En línea' : 'Sin conexión'}</span><strong>${state.status.syncing ? 'Sincronizando' : 'Listo'}</strong></div>
    <div class="status-pill"><span>Cambios pendientes</span><strong>${pending}</strong></div>
    <div class="status-pill"><span>Última sincronización</span><strong>${formatDate(state.lastSync)}</strong></div>
    <div class="status-pill"><span>Productos cargados</span><strong>${state.products.length}</strong></div>`;
}
