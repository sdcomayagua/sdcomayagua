(function(){
  ...
  function syncLocal(){state=SDCStore.load(); hydrateState(); state.unlocked=true; save(); applyAppearance(); render(); toast('Sincronizado con los datos guardados en este dispositivo.');}
  ...
  function unlock(){ if($('#keyInput').value.trim()===(state.settings.accessKey||'199311')){state.unlocked=true;save();render();toast('Acceso autorizado.')} else toast('Clave incorrecta.'); }
  function topbar(){const turbo=currentAppearance()==='turbo'; return `<header class="topbar"><img class="top-logo" src="${LOGO_SRC}" alt="SD"><div class="top-title"><h1>SD COMAYAGUA</h1><p>Ventas · inventario · cotizaciones</p></div><div class="spacer"></div><button class="btn small ghost aspect-btn" data-action="theme">${turbo?'Normal':'Pro'}</button><button class="btn small ghost sync-btn" data-action="sync">Sync</button><button class="btn small secondary" data-action="lock">Salir</button></header>`}
  function hero(){
    const st=stats();
